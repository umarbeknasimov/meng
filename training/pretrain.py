...
def load_pretrained(output_location, pretrained_step, model, optimizer, scheduler):
    pretrained_model = get_model_state_dict(paths.model(output_location, pretrained_step))
    pretrained_optim = get_optim_state_dict(paths.optim(output_location, pretrained_step))
