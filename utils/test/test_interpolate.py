...
class TestInterpolate(test_case.TestCase):
    ...
    def test_optimizer_average_state_dicts_with_all_empty_state(self):
        param_groups = {
            'lr': 0.1
        }
        optimizers = [{
    ...
    def test_optimizer_average_state_dicts(self):
        ...
        self.assertDictEqual(param_groups, result['param_groups'])
        self.assertTrue(np.array_equal(np.ones((2, 2)), result['state'][0]['momentum_1'].numpy()))
        self.assertTrue(np.array_equal(np.ones((3, 3)), result['state'][0]['momentum_2'].numpy()))
